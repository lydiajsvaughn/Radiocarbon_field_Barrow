...
ggsave("plots2014.png", combo, path="plots", width = 6,height = 8,dpi = 300)
```
...
ggsave("chamber14C_flux.png", path="plots", width = 6,height = 4, units="in", dpi = 300)
```
...
monthplot_type <- ggplot(highf %>% mutate(month = format(observation_date, "%B"), month = ordered(month, levels = c("July","August","September","October"))), aes(y=C14_atmcorr_permil, x=month)) +
  ...
  ylab(expression(Delta^14*C[Reco] ~ ("\u2030"))) 
  theme(axis.text.x = element_text(angle=90, size=11, color="black", vjust=0.5)) +
