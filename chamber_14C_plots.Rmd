...
thaw2014<-ggplot(summthaw, aes(x=type, y=mean.thaw)) +
  geom_bar(stat="identity", width=0.7) +
...
```{r}
p1 <- thaw2014 + boxtheme + theme(axis.title.y = element_text(vjust=2)) + theme(plot.margin = unit(c(3, 2, 3, 3), "mm"))
p2 <- flux2014 + boxtheme 
p3 <- bar2014 + boxtheme
combo <- grid.arrange(p1, p2, p3, nrow=3, heights=c(8,8,8))
ggsave("plots2014.png", combo, path="plots", width = 4.5,height =9,dpi = 300)
```
